...
// Hàm chia nhỏ tin nhắn nếu vượt quá giới hạn 4096 ký tự
const sendLongMessage = (chatId, text) => {
...
bot.on('message', (msg) => {
    ...
    if (text.startsWith('http') || text.startsWith('htttp') || text.startsWith('htttps')) {
        ...
        child.stdout.on('data', (data) => {
            ...
            console.log(`[DEBUG] stdout: ${data.toString()}`); // Debug vào build logs
        });
        child.stderr.on('data', (data) => {
            output += data.toString();
            console.log(`[DEBUG] stderr: ${data.toString()}`); // Debug vào build logs
        });
        child.on('close', () => {
            console.log(`[DEBUG] Lệnh đã kết thúc với kết quả: ${output}`); // Debug vào build logs
